...
class WebSocket {
    ...
    public function __construct($address, $port, $func = null) {
        if($func !== null) {
    ...
    public function process($user, $msg) {
        ...
        foreach($this->registery as $func) {
            $func($this, $user, $msg);
        }
    }
    ...
    public function calcKey($key1, $key2, $l8b) {
        //Get the numbers
...
$master = new WebSocket("localhost", 5876, function (WebSocket &$websocket, User &$user, $msg) {
    echo "msg: ", $msg, PHP_EOL; 
    if(!$user->logged) {
        $websocket->send($user->socket, 'login');
    }
